// 反混淆 AnimationHistory 类
class AnimationHistory {
    constructor() {
        this.current = null;           // 当前动画的历史数据（如物理模拟的速度、缓冲等）
        this.previous = null;          // 上一个动画的历史数据（用于动画混合时参考）
        this.currentTrackName = "";    // 当前动画轨道的名称
    }

    /**
     * 检查动画轨道是否变化，若变化则切换历史记录
     * @param {string} trackName - 当前动画轨道名
     * @param {object} animationObj - 当前动画对象，需有 createHistory 方法
     */
    check(trackName, animationObj) {
        if (trackName !== this.currentTrackName) {
            this.previous = this.current;
            this.current = animationObj.createHistory();
...
// 反混淆 Ct 类（基础动画参数对象）
class BaseAnimationParams {
    constructor(params = {}) {
        // 动画模式（如1:正弦，2:物理，3:wiggle等）
        this.mode = params.mode !== undefined ? params.mode : 1;
        // 动画名称
        this.name = params.name !== undefined ? params.name : "";
        // 动画延迟
        this.delay = params.delay !== undefined ? params.delay : 0;
    }
}

// 反混淆 FlashAnimationParams 类（闪烁动画参数对象）
class FlashAnimationParams extends BaseAnimationParams {
    constructor(params = {}) {
        super(params);
        // 闪烁周期（秒）
        this.blinkTime = params.blinkTime !== undefined ? params.blinkTime : 1;
        // 最小透明度
...
// 反混淆 TweenAnimationParams 类（补间动画参数对象）
class TweenAnimationParams extends BaseAnimationParams {
    constructor(params = {}) {
        super(params);
        // 补间关键帧数组
        this.frames = Array.isArray(params.frames) ? params.frames.map(frame => new Io(frame)) : [];
        // 补间动画结束时间
        this.framesEnd = this.frames.length > 0 ? this.frames[this.frames.length - 1].start : 0;
    }
}

// 反混淆 AutoBoneSpeedParams 类（自动骨骼速度参数对象）
class AutoBoneSpeedParams {
    constructor(params = {}) {
        // 时间缩放因子，影响自动骨骼动画的速度
        this.timeScale = params.timeScale !== undefined ? params.timeScale : 1;
    }
}


class CustomAutoSlotAnimator {
    constructor(params = {}, spineObj) {
        // 解析动画参数
        const animation = params.animation || {};
        const slotName = params.slotName || "";

        // 动画配置（每个动画名都实例化为动画对象）
        this.animation = Object.fromEntries(
            Object.entries(animation).map(([name, animParams]) => [
                name,
                CustomAutoSlotAnimator.createAnimation(animParams)
            ])
        );

        this.spineObj = spineObj;
        // 查找并设置目标slot
        this.slot = spineObj.skeleton.slots.find(slot => slot.data.name === slotName);
    }
    ...
    render(time) {
        ...
        if (currAnim.mode === 1) {
            this.updateSineMode(currAnim, time);
        } else if (currAnim.mode === 2) {
            this.updateTweenMode(currAnim, time);
        }
    }

    // 闪烁模式
    updateSineMode(params, time) {
        // 通过正弦函数控制slot的透明度，实现闪烁效果
        this.slot.color.a =
            (0.5 + 0.5 * Math.sin((time + params.delay) * Math.PI * 2 / params.blinkTime)) *
            (params.max - params.min) + params.min;
    }

    // 补间模式
    updateTweenMode(params, time) {
        if (params.framesEnd !== 0) {
            const t = (time + params.delay + 1000) % params.framesEnd;
            const block = Io.getBlock(params.frames, t);
            ...
            // 线性插值slot的透明度
            this.slot.color.a = (endValue - startValue) * (t - startTime) / (endTime - startTime) + startValue;
        }
    }

    // 当前动画对象
    get currentAnimation() {
        const name = this.spineObj.state.tracks[0].animation.name;
        return this.animation[name] || this.defaultAnimation;
    }
    ...
    static createAnimation(params) {
        switch (params.mode) {
            case 1: return new FlashAnimationParams(params); // 闪烁
            case 2: return new TweenAnimationParams(params); // 补间
            default: return new BaseAnimationParams(params); // 默认
        }
    }
}
...
class Animation {
    constructor(params = {}) {
        ...
        this.mode = params.mode || 1; // 动画模式
        
        this.initByMode(params, this.mode);
    }

    initByMode(params, mode) {
        if (mode === 1) { 
            // 旋转相关
            this.rotateOffset = params.rotateOffset !== undefined ? params.rotateOffset : 0;
            this.rotateTime = params.rotateTime !== undefined ? params.rotateTime : 2;
            this.rotateRange = params.rotateRange !== undefined ? params.rotateRange : 10;
            this.rotateCenter = params.rotateCenter !== undefined ? params.rotateCenter : 0;
            this.childOffset = params.childOffset !== undefined ? params.childOffset : 0.25;
            this.spring = params.spring !== undefined ? params.spring : 0;
            this.affectByLevel = params.affectByLevel !== undefined ? params.affectByLevel : 0.1;
            this.springLevel = params.springLevel !== undefined ? params.springLevel : 0;

            // scaleY相关
            this.scaleYRange = params.scaleYRange !== undefined ? params.scaleYRange : 0;
            this.scaleYTime = params.scaleYTime !== undefined ? params.scaleYTime : 2;
            this.scaleYCenter = params.scaleYCenter !== undefined ? params.scaleYCenter : 0;
            this.scaleYOffset = params.scaleYOffset !== undefined ? params.scaleYOffset : 0;
            this.scaleYChildOffset = params.scaleYChildOffset !== undefined ? params.scaleYChildOffset : 0.25;
            this.scaleYSpring = params.scaleYSpring !== undefined ? params.scaleYSpring : 0;
            this.scaleYAffectByLevel = params.scaleYAffectByLevel !== undefined ? params.scaleYAffectByLevel : 0.1;

            // scaleX相关
            this.scaleXRange = params.scaleXRange !== undefined ? params.scaleXRange : 0;
            this.scaleXTime = params.scaleXTime !== undefined ? params.scaleXTime : 2;
            this.scaleXCenter = params.scaleXCenter !== undefined ? params.scaleXCenter : 0;
            this.scaleXOffset = params.scaleXOffset !== undefined ? params.scaleXOffset : 0;
            this.scaleXChildOffset = params.scaleXChildOffset !== undefined ? params.scaleXChildOffset : 0.25;
            this.scaleXSpring = params.scaleXSpring !== undefined ? params.scaleXSpring : 0;
            this.scaleXAffectByLevel = params.scaleXAffectByLevel !== undefined ? params.scaleXAffectByLevel : 0.1;

            // scaleX和scaleY是否完全一致
            this.sinScaleXSameAsY =
                this.scaleXRange === this.scaleYRange &&
                ...
                this.scaleYAffectByLevel === this.scaleXAffectByLevel;

            // moveX相关
            this.moveXRange = params.moveXRange !== undefined ? params.moveXRange : 0;
            this.moveXTime = params.moveXTime !== undefined ? params.moveXTime : 2;
            this.moveXSpring = params.moveXSpring !== undefined ? params.moveXSpring : 0;
            this.moveXChildOffset = params.moveXChildOffset !== undefined ? params.moveXChildOffset : 0.25;
            this.moveXAffectByLevel = params.moveXAffectByLevel !== undefined ? params.moveXAffectByLevel : 0.1;
            this.moveXOffset = params.moveXOffset !== undefined ? params.moveXOffset : 0;
            this.moveXCenter = params.moveXCenter !== undefined ? params.moveXCenter : 0;

            // moveY相关
            this.moveYRange = params.moveYRange !== undefined ? params.moveYRange : 0;
            this.moveYTime = params.moveYTime !== undefined ? params.moveYTime : 2;
            this.moveYSpring = params.moveYSpring !== undefined ? params.moveYSpring : 0;
            this.moveYChildOffset = params.moveYChildOffset !== undefined ? params.moveYChildOffset : 0.25;
            this.moveYAffectByLevel = params.moveYAffectByLevel !== undefined ? params.moveYAffectByLevel : 0.1;
            this.moveYOffset = params.moveYOffset !== undefined ? params.moveYOffset : 0;
            this.moveYCenter = params.moveYCenter !== undefined ? params.moveYCenter : this.moveXCenter;
        } 
        if (mode === 3) {
            // X方向移动
            this.moveXFreq = params.moveXFreq !== undefined ? params.moveXFreq : 1;
            this.moveXAmp = params.moveXAmp !== undefined ? params.moveXAmp : 0;
            this.moveXOctaves = params.moveXOctaves !== undefined ? params.moveXOctaves : 0;
            this.moveXDelay = params.moveXDelay !== undefined ? params.moveXDelay : 0;
            this.moveXCenter = params.moveXCenter !== undefined ? params.moveXCenter : 0;
            this.moveXSeed = params.moveXSeed !== undefined ? params.moveXSeed : Math.floor(10000 * Math.random());

            // Y方向移动
            this.moveYFreq = params.moveYFreq !== undefined ? params.moveYFreq : this.moveXFreq;
            this.moveYAmp = params.moveYAmp !== undefined ? params.moveYAmp : this.moveXAmp;
            this.moveYOctaves = params.moveYOctaves !== undefined ? params.moveYOctaves : this.moveXOctaves;
            this.moveYDelay = params.moveYDelay !== undefined ? params.moveYDelay : this.moveXDelay;
            this.moveYCenter = params.moveYCenter !== undefined ? params.moveYCenter : this.moveXCenter;
            this.moveYSameAsX = (
                this.moveXFreq === this.moveYFreq &&
                this.moveXAmp === this.moveYAmp &&
                this.moveXOctaves === this.moveYOctaves &&
                this.moveXDelay === this.moveYDelay &&
                this.moveXCenter === this.moveYCenter
            );

            // X方向缩放
            this.scaleXFreq = params.scaleXFreq !== undefined ? params.scaleXFreq : 1;
            this.scaleXAmp = params.scaleXAmp !== undefined ? params.scaleXAmp : 0;
            this.scaleXOctaves = params.scaleXOctaves !== undefined ? params.scaleXOctaves : 0;
            this.scaleXDelay = params.scaleXDelay !== undefined ? params.scaleXDelay : 0;
            this.scaleXCenter = params.scaleXCenter !== undefined ? params.scaleXCenter : 0;

            // Y方向缩放
            this.scaleYFreq = params.scaleYFreq !== undefined ? params.scaleYFreq : this.scaleXFreq;
            this.scaleYAmp = params.scaleYAmp !== undefined ? params.scaleYAmp : this.scaleXAmp;
            this.scaleYOctaves = params.scaleYOctaves !== undefined ? params.scaleYOctaves : this.scaleXOctaves;
            this.scaleYDelay = params.scaleYDelay !== undefined ? params.scaleYDelay : this.scaleXDelay;
            this.scaleYCenter = params.scaleYCenter !== undefined ? params.scaleYCenter : this.scaleXCenter;
            this.scaleYSameAsX = (
                this.scaleXFreq === this.scaleYFreq &&
                this.scaleXAmp === this.scaleYAmp &&
                this.scaleXOctaves === this.scaleYOctaves &&
                this.scaleXDelay === this.scaleYDelay &&
                this.scaleXCenter === this.scaleYCenter
            );

            // 旋转
            this.rotateSpeed = params.rotateSpeed !== undefined ? params.rotateSpeed : 0;
            this.rotateFreq = params.rotateFreq !== undefined ? params.rotateFreq : 1;
            this.rotateAmp = params.rotateAmp !== undefined ? params.rotateAmp : 0;
            this.rotateOctaves = params.rotateOctaves !== undefined ? params.rotateOctaves : 0;
            this.rotateDelay = params.rotateDelay !== undefined ? params.rotateDelay : 0;
            this.rotateCenter = params.rotateCenter !== undefined ? params.rotateCenter : 0;
            this.rotateFollowLimit = params.rotateFollowLimit !== undefined ? params.rotateFollowLimit : 0;
            this.rotateFollowSpeed = params.rotateFollowSpeed !== undefined ? params.rotateFollowSpeed : 0.1;
            this.rotateFollowFlip = params.rotateFollowFlip !== undefined ? params.rotateFollowFlip : 0;
            this.rotateFollowXMax = params.rotateFollowXMax !== undefined ? params.rotateFollowXMax : 20;
            this.rotateFollowYMax = params.rotateFollowYMax !== undefined ? params.rotateFollowYMax : 20;
            this.rotateFollowEnable = this.rotateFollowLimit !== 0;
        }
        if (mode === 4) {
            this.delay = params.delay !== undefined ? params.delay : 0.1;
            this.speed = params.speed !== undefined ? params.speed : 0.1;
            this.spring = params.spring !== undefined ? params.spring : 0;
            this.springRot = params.springRot !== undefined ? params.springRot : 0;
            this.affectByLevel = params.affectByLevel !== undefined ? params.affectByLevel : 0;
            this.springLevel = params.springLevel !== undefined ? params.springLevel : 0;
            this.limitRange = params.limitRange !== undefined ? params.limitRange : 80;
            this.rotateOffset = params.rotateOffset !== undefined ? params.rotateOffset : 0;
            this.friction = params.friction !== undefined ? params.friction : 0.7;
            this.springUseTarget = params.springUseTarget !== undefined ? params.springUseTarget : false;
            this.windX = params.windX !== undefined ? params.windX : 0;
            this.windY = params.windY !== undefined ? params.windY : 0;
            this.windFreq = params.windFreq !== undefined ? params.windFreq : 1;
            this.windAccel = params.windAccel !== undefined ? params.windAccel : 1;
            this.windDelay = params.windDelay !== undefined ? params.windDelay : 0;
            this.windOctaves = params.windOctaves !== undefined ? params.windOctaves : 0;
...
class CustomAutoBoneAnimator {
    constructor(config = {}, spineObj) {
        // 动画参数表（每个动画名对应一个动画参数对象）
        const animationMap = config.animation || {};
        this.animation = Object.fromEntries(
            Object.entries(animationMap).map(([name, animConfig]) => {
                return [name, this.createAnimation(animConfig)];
            })
        );
        ...
        // 根骨骼名
        this.rootBoneName = config.rootBoneName || "";
        // 末端骨骼名（数组）
        this.endBoneName = Array.isArray(config.endBoneName)
            ? config.endBoneName
            : [config.endBoneName].filter(name => name !== "");

        // 跟随目标骨骼名
        this.targetBoneName = config.targetBoneName || "";
        // 跟随目标末端骨骼名
        this.targetEndBoneName = config.targetEndBoneName || "";
        // 跟随权重
        this.targetWeight = config.targetWeight !== undefined ? config.targetWeight : 1;
    ...
    init(bone, level = 0) {
        ...
        bone.children.forEach(child => {
            this.init(child, level + 1);
        });
        // 如果是叶子骨骼（没有子骨骼），初始化尾部的前一帧世界坐标
        if (bone.children.length === 0) {
            bone.tailAutoMovePrevWorldX = bone.y * bone.b + bone.worldX;
            bone.tailAutoMovePrevWorldY = bone.y * bone.d + bone.worldY;
        }

        // this.initByMode(bone, this.animation.mode);
    }

    bind(skeletonMesh) {
        // 绑定skeletonMesh对象到自定义动画实例
        this.skeletonMesh = skeletonMesh;
        // 查找并设置根骨骼
        this.rootBone = skeletonMesh.skeleton.findBone(this.rootBoneName);
        // 如果有目标骨骼名，查找并设置目标骨骼
        if (this.targetBoneName !== "") {
            this.targetBone = skeletonMesh.skeleton.findBone(this.targetBoneName);
        }
        // 初始化骨骼初始状态（递归所有子骨骼）
    ...
    resetBone(bone = this.rootBone) {
        ...
        if (!this.endBoneName.includes(bone.name)) {
            bone.children.forEach(child => {
                this.resetBone(child);
            });
        }
    }

    render(delta, time, mix, prevTrackName) {
        // delta: 当前帧的delta时间
        // time: 当前动画时间
        // mix: 混合因子
        // prevTrackName: 上一动画名（用于混合过渡）

        let prevAnim = null;
        const currAnim = this.currentAnimation;
        const currTrackName = this.currentTrackName;
        let blend = 1;

        // 检查历史记录（用于物理模式等）
        if (!this.history.current) {
            this.history.check(this.currentTrackName, currAnim);
        }

        // 如果有前一动画且动画名发生变化，处理混合
        if (prevTrackName && currTrackName !== prevTrackName) {
            prevAnim = this.animation[prevTrackName] || this.defaultAnimation;
            this.history.check(this.currentTrackName, currAnim);
        }

        // 如果有前一动画且混合因子不为1，先渲染前一动画（混合过渡）
        if (prevAnim && mix !== 1) {
            blend = mix;
            this.renderAutoBone(prevAnim, this.history.previous, delta, time, 1);
        }

        // 渲染当前动画
        this.renderAutoBone(currAnim, this.history.current, delta, time, blend);
    }
    ...
    updateWiggleMode(
        frequency, amplitude, octaves, time, phase, persistence = 0.5
    ) {
        let sum = 0;
        let weight = 1;
        let layers = octaves + 1;
        let baseFreq = 1 / (2 - 1 / Math.pow(2, layers - 1));
        let freq = baseFreq;
        let totalWeight = 0;
        for (let i = 0; i < layers; i++) {
            sum += weight * Math.sin(time * freq * Math.PI * 2 / frequency + phase);
            freq = baseFreq * Math.pow(1.5, i + 1);
            totalWeight += weight;
            weight *= persistence;
        }
        return (sum / totalWeight) * amplitude;
    }

    updateSineMode(
        params, time, bone = this.rootBone, targetBone = this.targetBone, level = 0, mix
    ) {
        const self = this;
        if (!this.endBoneName.includes(bone.data.name)) {
            const hasTarget = targetBone && targetBone.data.name !== this.targetEndBoneName;
            const baseRotation = hasTarget
            ...
            bone.rotation = this.mixValue(
                ...
                baseRotation +
                    Math.sin(
                        (params.rotateOffset - Math.pow(params.childOffset * level, 1 + params.spring) + time) *
                            Math.PI * 2 / params.rotateTime
                    ) *
                        params.rotateRange *
                        Math.pow(1 + level * params.affectByLevel, 1 + params.springLevel) +
                    params.rotateCenter,
                mix
            );
            ...
            if (params.scaleYRange !== 0) {
                const baseScaleY = hasTarget
                    ? this.mixValue(bone.initScaleY, targetBone.scaleY, this.targetWeight)
                    : bone.initScaleY;
                scaleYValue =
                    Math.sin(
                        (params.scaleYOffset - Math.pow(params.scaleYChildOffset * level, 1 + params.scaleYSpring) + time) *
                            Math.PI * 2 / params.scaleYTime
                    ) *
                        params.scaleYRange *
                        Math.pow(1 + level * params.scaleYAffectByLevel, 1 + params.springLevel) +
                    params.scaleYCenter;
                bone.scaleY = this.mixValue(bone.scaleY, baseScaleY + scaleYValue, mix);

                if (params.sinScaleXSameAsY) {
                    const baseScaleX = hasTarget
                        ? this.mixValue(bone.initScaleX, targetBone.scaleX, this.targetWeight)
                        : bone.initScaleX;
                    bone.scaleX = this.mixValue(bone.scaleX, baseScaleX + scaleYValue, mix);
                }
            }
            if (!params.sinScaleXSameAsY && params.scaleXRange !== 0) {
                ...
                const scaleXValue =
                    Math.sin(
                        (params.scaleXOffset - Math.pow(params.scaleXChildOffset * level, 1 + params.scaleXSpring) + time) *
                            Math.PI * 2 / params.scaleXTime
                    ) *
                        params.scaleXRange *
                        Math.pow(1 + level * params.scaleXAffectByLevel, 1 + params.springLevel) +
                    params.scaleXCenter;
                bone.scaleX = this.mixValue(bone.scaleX, baseScaleX + scaleXValue, mix);
            }

            // X/Y 平移
            if (params.moveXRange !== 0) {
                const baseX = hasTarget
                    ? this.mixValue(bone.initX, targetBone.x, this.targetWeight)
                    : bone.initX;
                const moveXValue =
                    Math.sin(
                        (params.moveXOffset - Math.pow(params.moveXChildOffset * level, 1 + params.moveXSpring) + time) *
                            Math.PI * 2 / params.moveXTime
                    ) *
                        params.moveXRange *
                        Math.pow(1 + level * params.moveXAffectByLevel, 1 + params.springLevel) +
                    params.moveXCenter;
                bone.x = this.mixValue(bone.x, baseX + moveXValue, mix);
            }
            if (params.moveYRange !== 0) {
                const baseY = hasTarget
                    ? this.mixValue(bone.initY, targetBone.y, this.targetWeight)
                    : bone.initY;
                const moveYValue =
                    Math.sin(
                        (params.moveYOffset - Math.pow(params.moveYChildOffset * level, 1 + params.moveYSpring) + time) *
                            Math.PI * 2 / params.moveYTime
                    ) *
                        params.moveYRange *
                        Math.pow(1 + level * params.moveYAffectByLevel, 1 + params.springLevel) +
                    params.moveYCenter;
                bone.y = this.mixValue(bone.y, baseY + moveYValue, mix);
            }

            // 递归对子骨骼应用
            bone.children.forEach(function(childBone, idx) {
                const childTargetBone = hasTarget ? targetBone.children[idx] : null;
                self.updateSineMode(params, time, childBone, childTargetBone, level + 1, mix);
            });
        }
    }

    /**
     * 递归对子骨骼应用物理模式的“跟随”动画（用于层级骨骼的惯性/延迟旋转）。
     * @param {object} params - 动画参数对象，需包含 delay、spring、rotateMoveRange、affectByLevel、springLevel 等
     * @param {object} history - 历史记录对象，需包含 buffer（历史旋转增量数组）
     * @param {object} bone - 当前骨骼对象
     * @param {number} time - 当前动画时间
     * @param {number} level - 当前递归层级
     * @param {number} mix - 混合因子
     */
    updateFollowMode(params, history, bone, time, level, mix) {
        // 如果是终止骨骼则不处理
        if (this.endBoneName.includes(bone.data.name)) return;

        // 计算插值后的旋转增量
        const historyTime = time - params.delay * (1 + level * params.spring);
        const delta = this.getHistoryRotate(historyTime, history.buffer);

        // 计算层级影响
        const affect = Math.pow(1 + level * params.affectByLevel, 1 + params.springLevel);

        // 应用旋转（带混合）
        bone.rotation = this.mixValue(
            bone.rotation,
            bone.initRotation + delta * params.rotateMoveRange * affect,
            mix
        );

        // 递归对子骨骼应用
        bone.children.forEach(childBone => {
            this.updateFollowMode(params, history, childBone, time, level + 1, mix);
        });
    }

    updatePhysicMode(params, history, bone, time, delta, mix) {
        const self = this;
        const dx = Math.min(params.limitRange, Math.max(-params.limitRange, bone.autoMovePrevWorldX - bone.worldX));
        const dy = Math.min(params.limitRange, Math.max(-params.limitRange, bone.autoMovePrevWorldY - bone.worldY));
        history.speedX += (params.affectByX * dx - history.speedX) * params.speed * delta;
        history.speedY += (params.affectByY * dy - history.speedY) * params.speed * delta;
        bone.autoMovePrevWorldX = bone.worldX;
        bone.autoMovePrevWorldY = bone.worldY;
        const rotationDelta = params.affectByRange * (-history.speedX * bone.c + history.speedY * bone.d);
        bone.rotation = this.mixValue(bone.rotation, rotationDelta + bone.initRotation, mix);
        history.buffer.push({
            time: time,
            delta: rotationDelta
        });
        ...
        bone.children.forEach(function(childBone) {
            self.updateFollowMode(params, history, childBone, time, 1, mix);
        });
    }

    updateElasic(params, bone, delta, mix) {
        // params: 弹性动画参数对象
        // bone: 当前骨骼
        // delta: 帧间隔
        // mix: 混合因子
        ...
        if (!this.endBoneName.includes(bone.data.name)) {
            ...
            // 计算骨骼初始点在父骨骼世界坐标系下的位置
            const worldInitX = initX * parent.a + initY * parent.b + parent.worldX;
            const worldInitY = initX * parent.c + initY * parent.d + parent.worldY;

            // 计算弹性速度
            const elasticDeltaX = (worldInitX - bone.autoMovePrevWorldX) * params.elasticSpring * delta;
            const elasticDeltaY = (worldInitY - bone.autoMovePrevWorldY) * params.elasticSpringY * delta;

            bone.elasticSpeedX += elasticDeltaX;
            bone.elasticSpeedX *= params.elasticFriction;
            bone.elasticSpeedY += elasticDeltaY;
            bone.elasticSpeedY *= params.elasticFrictionY;
            ...
            if (!isNaN(localX) && !isNaN(localY)) {
                bone.x = this.mixValue(
                    bone.x,
                    localX * params.elasticSoftness + (1 - params.elasticSoftness) * bone.initX,
                    mix * bone.autoMoveFriction
                );
                bone.y = this.mixValue(
                    bone.y,
                    localY * params.elasticSoftnessY + (1 - params.elasticSoftnessY) * bone.initY,
                    mix * bone.autoMoveFriction
                );
    ...
    updateSpringMagic(params, bone, targetBone, time, delta, level, mix, direction) {
        // params: 动画参数对象
        // bone: 当前骨骼
        // targetBone: 目标骨骼
        // time: 当前时间
        ...
        if (!this.endBoneName.includes(bone.data.name)) {
            ...
            const baseRotation = hasTarget
                ? this.mixValue(bone.initRotation, targetBone.rotation, this.targetWeight)
                : bone.initRotation;
            const springBone = params.springUseTarget && targetBone ? targetBone : bone;

            // 计算递归层级影响
            const affectLevel = 1 + level * params.affectByLevel;
            const affectLevelPow = Math.pow(affectLevel, 1 + params.springLevel);

            // 计算延迟、摩擦、风力等参数
            const delay = params.delay * affectLevelPow * (1 + params.springRot * affectLevel) * delta * (level === 0 ? 1 + params.spring : 1);
            const friction = params.friction;
            const forceX = params.forceX;
            const forceY = params.forceY;
            const windDecay = 1 - params.windAccel;

            if (bone.children.length > 0) {
                bone.children.forEach(function(childBone, idx) {
                    if (idx === 0) {
                        // 只对第一个子骨骼应用弹簧主逻辑
                        const localX = childBone.x;
                        const localY = childBone.y;
                        // 计算子骨骼的世界坐标
                        const worldX = localX * springBone.a + localY * springBone.b + bone.worldX;
                        const worldY = localX * springBone.c + localY * springBone.d + bone.worldY;
                        // 计算速度变化
                        let vx = (worldX - childBone.autoMovePrevWorldX) * delay;
                        let vy = (worldY - childBone.autoMovePrevWorldY) * delay;
                        bone.autoMoveSpeedX += vx;
                        bone.autoMoveSpeedY += vy;
                        bone.autoMoveSpeedX *= friction;
                        bone.autoMoveSpeedY *= friction;
                        bone.autoMoveSpeedX += forceX * params.windAccel;
                        bone.autoMoveSpeedY += forceY * params.windAccel;
                        // 计算新的世界坐标
                        const nextWorldX = childBone.autoMovePrevWorldX + bone.autoMoveSpeedX + forceX * windDecay;
                        const nextWorldY = childBone.autoMovePrevWorldY + bone.autoMoveSpeedY + forceY * windDecay;
                        // 计算旋转
                        const angle = bone.worldToLocalRotation(
                            direction * Math.atan2(nextWorldY - bone.worldY, direction * (nextWorldX - bone.worldX)) * 180 / Math.PI +
                            (level === 0 ? params.rotateOffset : 0)
                        );
                        const limitedAngle = Math.min(params.limitRange, Math.max(-params.limitRange, angle - baseRotation)) + baseRotation;
                        bone.rotation = self.mixValue(
                            bone.rotation,
                            baseRotation * params.speed + (1 - params.speed) * limitedAngle,
                            mix * bone.autoMoveFriction
                        );
                        bone.updateWorldTransform();
                    }
                    // 递归对子骨骼应用
                    const childTargetBone = hasTarget ? targetBone.children[idx] : null;
                    self.updateSpringMagic(params, childBone, childTargetBone, time, delta, level + 1, mix, direction);
                });
            } else {
                // 叶子骨骼（如尾巴末端）
                const localX = bone.x;
                ...
                const worldX = localX * springBone.a + localY * springBone.b + bone.worldX;
                const worldY = localX * springBone.c + localY * springBone.d + bone.worldY;
                let vx = (worldX - bone.tailAutoMovePrevWorldX) * delay;
                let vy = (worldY - bone.tailAutoMovePrevWorldY) * delay;
                bone.autoMoveSpeedX += vx;
                bone.autoMoveSpeedY += vy;
                bone.autoMoveSpeedX *= friction;
                bone.autoMoveSpeedY *= friction;
                bone.autoMoveSpeedX += forceX * params.windAccel;
                bone.autoMoveSpeedY += forceY * params.windAccel;
                const nextWorldX = bone.tailAutoMovePrevWorldX + bone.autoMoveSpeedX + forceX * windDecay;
                const nextWorldY = bone.tailAutoMovePrevWorldY + bone.autoMoveSpeedY + forceY * windDecay;
                const angle = bone.worldToLocalRotation(
                    direction * Math.atan2(nextWorldY - bone.worldY, direction * (nextWorldX - bone.worldX)) * 180 / Math.PI +
                    (level === 0 ? params.rotateOffset : 0)
                );
    ...
    updateKeyFrameMode(params, bone, time, mix) {
        // params: 关键帧动画参数对象
        // bone: 当前骨骼
        // time: 当前时间
        // mix: 混合因子
        ...
        // 帧时间偏移
        const frameTime = time + params.delay + 1000;

        // x 关键帧
        if (params.xFramesEnd > 0) {
            const t = frameTime % params.xFramesEnd;
            const block = Io.getBlock(params.xFrames, t);
            ...
            const easeFunc = block.end.easeFunc;
            const v = (endValue - startValue) * easeFunc((t - startTime) / (endTime - startTime)) + startValue;
            bone.x = this.mixValue(bone.x, v, mix);
        }

        // y 关键帧
        if (params.yFramesEnd > 0) {
            const t = frameTime % params.yFramesEnd;
            const block = Io.getBlock(params.yFrames, t);
            const startValue = block.start.value;
            const endValue = block.end.value;
            const startTime = block.start.start;
            const endTime = block.end.start;
            const easeFunc = block.end.easeFunc;
            const v = (endValue - startValue) * easeFunc((t - startTime) / (endTime - startTime)) + startValue;
            bone.y = this.mixValue(bone.y, v, mix);
        }

        // scaleX 关键帧
        if (params.sxFramesEnd > 0) {
            const t = frameTime % params.sxFramesEnd;
            const block = Io.getBlock(params.sxFrames, t);
            const startValue = block.start.value;
            const endValue = block.end.value;
            const startTime = block.start.start;
            const endTime = block.end.start;
            const easeFunc = block.end.easeFunc;
            const v = (endValue - startValue) * easeFunc((t - startTime) / (endTime - startTime)) + startValue;
            bone.scaleX = this.mixValue(bone.scaleX, v, mix);

            // scaleY 同步 scaleX
            if (params.sYSameAsSX) {
                bone.scaleY = bone.scaleX;
            } else if (params.syFramesEnd > 0) {
                // scaleY 独立关键帧
                const tY = frameTime % params.syFramesEnd;
                const blockY = Io.getBlock(params.syFrames, tY);
                const startValueY = blockY.start.value;
                const endValueY = blockY.end.value;
                const startTimeY = blockY.start.start;
                const endTimeY = blockY.end.start;
                const easeFuncY = blockY.end.easeFunc;
                const vY = (endValueY - startValueY) * easeFuncY((tY - startTimeY) / (endTimeY - startTimeY)) + startValueY;
                bone.scaleY = this.mixValue(bone.scaleY, vY, mix);
            }
        }
    }

    renderAutoBone(animation, prevHistory, delta, time, mix) {
        const mode = animation.mode;
        if (mode === 1) {
            // 正弦模式
            this.updateSineMode(animation, time, this.rootBone, this.targetBone, 0, mix);
        } else if (mode === 2) {
            // 物理模式
            this.updatePhysicMode(animation, prevHistory, this.rootBone, time, delta, mix);
        } else if (mode === 3) {
            // wiggle模式
            // X方向
            const moveX = animation.moveXFreq,
                moveXAmp = animation.moveXAmp,
                moveXOctaves = animation.moveXOctaves,
                moveXDelay = animation.moveXDelay,
                moveXCenter = animation.moveXCenter,
                moveYSameAsX = animation.moveYSameAsX,
                moveXSeed = animation.moveXSeed;

            let wiggleX = moveXAmp === 0 ? 0 : this.updateWiggleMode(moveX, moveXAmp, moveXOctaves, time, moveXDelay) + moveXCenter;
            this.rootBone.x = this.mixValue(this.rootBone.x, this.rootBone.initX + wiggleX, mix);

            // Y方向
            if (moveYSameAsX) {
                wiggleX = moveXAmp === 0 ? 0 : this.updateWiggleMode(moveX, moveXAmp, moveXOctaves, time, moveXDelay + moveXSeed) + moveXCenter;
                this.rootBone.y = this.mixValue(this.rootBone.y, this.rootBone.initY + wiggleX, mix);
            } else {
                const moveY = animation.moveYFreq,
                    moveYAmp = animation.moveYAmp,
                    moveYOctaves = animation.moveYOctaves,
                    moveYDelay = animation.moveYDelay,
                    moveYCenter = animation.moveYCenter;
                const wiggleY = moveYAmp === 0 ? 0 : this.updateWiggleMode(moveY, moveYAmp, moveYOctaves, time, moveYDelay) + moveYCenter;
                this.rootBone.y = this.mixValue(this.rootBone.y, this.rootBone.initY + wiggleY, mix);
            }

            // scaleX
            const scaleXFreq = animation.scaleXFreq,
                scaleXAmp = animation.scaleXAmp,
                scaleXOctaves = animation.scaleXOctaves,
                scaleXDelay = animation.scaleXDelay,
                scaleXCenter = animation.scaleXCenter,
                scaleYSameAsX = animation.scaleYSameAsX;
            let wiggleScaleX = scaleXAmp === 0 ? 0 : this.updateWiggleMode(scaleXFreq, scaleXAmp, scaleXOctaves, time, scaleXDelay) + scaleXCenter;
            this.rootBone.scaleX = this.mixValue(this.rootBone.scaleX, this.rootBone.initScaleX + wiggleScaleX, mix);

            // scaleY
            if (scaleYSameAsX) {
                this.rootBone.scaleY = this.mixValue(this.rootBone.scaleY, this.rootBone.initScaleY + wiggleScaleX, mix);
            } else {
                const scaleYFreq = animation.scaleYFreq,
                    scaleYAmp = animation.scaleYAmp,
                    scaleYOctaves = animation.scaleYOctaves,
                    scaleYDelay = animation.scaleYDelay,
                    scaleYCenter = animation.scaleYCenter;
                const wiggleScaleY = scaleYAmp === 0 ? 0 : this.updateWiggleMode(scaleYFreq, scaleYAmp, scaleYOctaves, time, scaleYDelay) + scaleYCenter;
                this.rootBone.scaleY = this.mixValue(this.rootBone.scaleY, this.rootBone.initScaleY + wiggleScaleY, mix);
            }

            // rotation
            const rotateSpeed = animation.rotateSpeed,
                rotateFreq = animation.rotateFreq,
                rotateAmp = animation.rotateAmp,
                rotateOctaves = animation.rotateOctaves,
                rotateDelay = animation.rotateDelay,
                rotateCenter = animation.rotateCenter,
                rotateFollowEnable = animation.rotateFollowEnable,
                rotateFollowLimit = animation.rotateFollowLimit,
                rotateFollowSpeed = animation.rotateFollowSpeed,
                rotateFollowFlip = animation.rotateFollowFlip,
                rotateFollowXMax = animation.rotateFollowXMax,
                rotateFollowYMax = animation.rotateFollowYMax;

            let rotation = this.rootBone.initRotation + time * rotateSpeed * 360 + rotateCenter;
            rotation += rotateAmp === 0 ? 0 : this.updateWiggleMode(rotateFreq, rotateAmp, rotateOctaves, time, rotateDelay);

            if (rotateFollowEnable) {
                const dx = this.rootBone.worldX - this.rootBone.autoMovePrevWorldX;
                const dy = this.rootBone.worldY - this.rootBone.autoMovePrevWorldY;
                if (rotateFollowFlip === 1) {
                    // 保持原有分量跟随逻辑
                    let followDelta = -rotateFollowLimit * Math.max(-1, Math.min(1, dx / rotateFollowXMax))
                                    - rotateFollowLimit * Math.max(-1, Math.min(1, dy / rotateFollowYMax));
                    this.rootBone.followRotation += followDelta * rotateFollowSpeed;
                } else {
                    // 角度跟随，始终走最短路径
                    let targetAngle = Math.atan2(dy, dx) * 180 / Math.PI;
                    let diff = shortestAngleDiff(this.rootBone.followRotation, targetAngle);
                    diff = Math.max(-rotateFollowLimit, Math.min(rotateFollowLimit, diff));
                    this.rootBone.followRotation += diff * rotateFollowSpeed;
                }
                // 不要每帧归一化到0~360，保持连续性，只有在显示或输出时需要归一化
                if (rotateFollowFlip === 2 && Math.abs(((this.rootBone.followRotation + 360) % 360) - 180) < 90) {
            ...
            this.rootBone.rotation = this.mixValue(this.rootBone.rotation, rotation, mix);

        } else if (mode === 4) {
            // 弹簧/风力/重力模式
            const scale = this.rootBone.getWorldScale();
            const scaleX = scale.x;
            const scaleY = scale.y;
            const hasWindForce = animation.hasWindForce,
                gravityX = animation.gravityX,
                gravityY = animation.gravityY;
            animation.forceX = -gravityX;
            animation.forceY = -gravityY;
            if (hasWindForce) {
                const windFactor = 0.5 + 0.5 * this.updateWiggleMode(1 / animation.windFreq, 1, animation.windOctaves, time, animation.windDelay, 0.8);
                animation.forceX += animation.windX * windFactor;
                animation.forceY += animation.windY * windFactor;
            }
            this.updateSpringMagic(animation, this.rootBone, this.targetBone, time, delta, 0, mix, scaleX * scaleY < 0 ? -1 : 1);
        } else if (mode === 5) {
            // 弹性模式
            this.updateElasic(animation, this.rootBone, delta, mix);
        } else if (mode === 6) {
            // 关键帧模式
            this.updateKeyFrameMode(animation, this.rootBone, time, mix);
        }
    }

    // 建议加在类外部或类内部
    shortestAngleDiff(a, b) {
        // 返回 b 到 a 的最短角度差（单位：度，范围 -180~180）
        let diff = (b - a + 540) % 360 - 180;
        return diff;
    }

    /**
     * 在物理模式下，根据历史记录缓冲区，获取指定时间点的旋转增量（带插值）。
     * @param {number} time - 查询的时间点
     * @param {Array} buffer - 历史记录数组，每项包含 { time, delta }
     * @returns {number} - 插值后的旋转增量
     */
    getHistoryRotate(time, buffer) {
        // 从后往前查找，找到第一个记录点p，其时间大于time
        for (let i = buffer.length - 1; i >= 0; i--) {
            const p = buffer[i];
            if (p.time > time) {
                // 再往前找，找到第一个记录点R，其时间小于等于time
                for (let j = i - 1; j >= 0; j--) {
                    const R = buffer[j];
                    if (time >= R.time) {
                        // 在R和p之间做线性插值
                        return R.delta + (p.delta - R.delta) * (time - R.time) / (p.time - R.time);
                    }
                }
                // 没有找到更早的点，返回0
                return 0;
    ...
    createAnimation(animation) {
        switch (animation.mode) {
            case 1:
                return new Animation(animation);      // 正弦模式
            case 2:
                return new PhysicAnimationParams(animation);    // 物理模式
            case 3:
                // return new WiggleAnimationParams(params);    // 抖动模式
                return new Animation(animation);
            case 4:
                return new Animation(animation);        // 弹簧/风力/重力模式
            case 5:
                return new ElasticAnimationParams(animation);   // 弹性模式
            case 6:
                return new KeyFrameAnimationParams(animation);  // 关键帧模式
            default:
                return new BaseAnimationParams(animation);      // 默认动画参数对象
        }
    }

    // 当前动画名
    get currentAnimation() {
        const tracks = this.skeletonMesh.state.tracks;
        if (!tracks || tracks.length === 0 || !tracks[0].animation) {
            return this.defaultAnimation;
        }
        const name = tracks[0].animation.name;
        return this.animation[name] || this.defaultAnimation;
    }
    // 当前动画对象
    get currentTrackName() {
        ...
        return tracks[0].animation.name;
    }
    // 默认动画对象
    get defaultAnimation() {
        return this.animation.default;
...
/**
 * 自定义Spine动画控制器 - 支持extra字段多骨骼独立动画
 */
class CustomSpineAnimationController {
    /**
     * @param {SkeletonMesh} skeletonMesh - spine-three.js的SkeletonMesh实例
     * @param {Object} extra - JSON中的extra字段
     */
    constructor(skeletonMesh, extra) {
        this.skeletonMesh = skeletonMesh;
        this.skeleton = skeletonMesh.skeleton;
        this.extraConfig = extra || {};
        this.animationTime = 0;
        this.boneConfigMap = new Map();
        this.initBoneConfigs();
    }

    // 初始化骨骼配置映射表
    initBoneConfigs() {
        Object.values(this.extraConfig).forEach(boneItem => {
            const boneName = boneItem.rootBoneName;
            const animationConfig = boneItem.animation?.default || {};
            if (boneName && animationConfig) {
                this.boneConfigMap.set(boneName, {
                    ...animationConfig,
                    prevWorldX: 0,
                    prevWorldY: 0
                });
    ...
    update(deltaTime) {
        ...
        this.applyAllBonesAnimation();
        this.skeleton.updateWorldTransform();
        this.skeletonMesh.updateGeometry();
    }

	applyAllBonesAnimation() {
		for (const bone of this.skeleton.bones) {
			const boneName = bone.data.name;
			const config = this.boneConfigMap.get(boneName);
			if (!config) continue;

			// 位移
			try {
				this.applyTranslation(bone, config);
				// console.log(`[Translation] ${boneName}: x=${bone.x}`);
			} catch (e) {
				console.warn(`[Translation Error] ${boneName}:`, e);
			}

			// 缩放
			try {
				this.applyScale(bone, config);
				// console.log(`[Scale] ${boneName}: scaleX=${bone.scaleX}`);
			} catch (e) {
				console.warn(`[Scale Error] ${boneName}:`, e);
			}

			// 旋转
			try {
				this.applyRotation(bone, config);
				// console.log(`[Rotation] ${boneName}: rotation=${bone.rotation}`);
			} catch (e) {
				console.warn(`[Rotation Error] ${boneName}:`, e);
			}

			// 记录世界坐标
			config.prevWorldX = bone.worldX;
			config.prevWorldY = bone.worldY;

			// 可选：调试输出
			if (this.debugMode) {
				console.log(`[Bone] ${boneName}: x=${bone.x}, rotation=${bone.rotation}, scaleX=${bone.scaleX}, time=${this.animationTime}`);
			}
		}
	}

    // 位移动画
	applyTranslation(bone, config) {
		const {
			moveXFreq = 0,
			moveXAmp = 0,
			moveXDelay = 0,
			moveXCenter = 0,
			moveXOctaves = 0,
			moveXSeed = Math.random()
		} = config;

		if (moveXAmp <= 0 || moveXFreq <= 0) return;

		const animTime = this.animationTime + moveXDelay;
		let xOffset = Math.sin(animTime * moveXFreq * Math.PI * 2) * moveXAmp;

		if (moveXOctaves > 0) {
			xOffset += this.getPerlinNoise(moveXSeed, animTime, moveXOctaves) * moveXAmp * 0.3;
		}

		// 修正：加上骨骼初始本地坐标
		bone.x = bone.data.x + moveXCenter + xOffset;
	}

    // 缩放动画
    applyScale(bone, config) {
        const {
            scaleXFreq = 0,
            scaleXAmp = 0,
            scaleXDelay = 0,
        ...
        if (scaleXAmp <= 0 || scaleXFreq <= 0) return;

        const animTime = this.animationTime + scaleXDelay;
        let scaleOffset = Math.cos(animTime * scaleXFreq * Math.PI * 2) * scaleXAmp;

        if (scaleXOctaves > 0) {
            scaleOffset += this.getPerlinNoise(scaleXCenter, animTime, scaleXOctaves) * scaleXAmp * 0.2;
        }

        bone.scaleX = scaleXCenter + 1 + scaleOffset;
    }

    // 旋转动画
    applyRotation(bone, config) {
        const {
            rotateSpeed = 0,
            rotateFreq = 0,
            rotateAmp = 0,
            ...
            rotateOctaves = 0,
            rotateFollowFlip = 0,
            rotateFollowLimit = 90,
            rotateFollowSpeed = 0.1,
            rotateFollowXMax = 20,
            rotateFollowYMax = 20
        } = config;

        let rotation = rotateSpeed * this.animationTime;

        if (rotateAmp > 0 && rotateFreq > 0) {
            const animTime = this.animationTime + rotateDelay;
            rotation += Math.sin(animTime * rotateFreq * Math.PI * 2) * rotateAmp;
        }

        if (rotateOctaves > 0) {
            rotation += this.getPerlinNoise(bone.data.name, this.animationTime, rotateOctaves) * rotateAmp * 0.5;
        }

        if (rotateFollowFlip) {
            const deltaX = bone.worldX - config.prevWorldX;
            const deltaY = bone.worldY - config.prevWorldY;
            const followRotX = (deltaX / rotateFollowXMax) * rotateFollowLimit;
            const followRotY = (deltaY / rotateFollowYMax) * rotateFollowLimit;
            rotation += (followRotX + followRotY) * rotateFollowSpeed;
        }

        bone.rotation = Math.max(-rotateFollowLimit, Math.min(rotateFollowLimit, rotation));
    }

    // 简易柏林噪声
    getPerlinNoise(seed, time, octaves) {
        let value = 0;
        let amplitude = 1;
        let frequency = 1;
        const seedNum = typeof seed === 'string' ? this.hashCode(seed) : seed;

        for (let i = 0; i < octaves; i++) {
            const t = time * frequency + seedNum;
            value += Math.sin(t) * amplitude;
            amplitude *= 0.5;
            frequency *= 2;
        }
        return value / (2 - amplitude);
    }
...
/**
 * 初始化自定义动画并集成到Spine流程
 * @param {SkeletonMesh} skeletonMesh - Spine网格实例
 * @param {Object} extraConfig - JSON中的extra配置
 * @returns {CustomSpineAnimationController} 动画控制器实例
 */
function initCustomSpineAnimation(skeletonMesh, extraConfig) {
    const customController = new CustomSpineAnimationController(skeletonMesh, extraConfig);

    // 保存原生update方法，注入自定义逻辑
    const originalUpdate = skeletonMesh.update;
    skeletonMesh.update = function(deltaTime) {
        // originalUpdate.call(this, deltaTime);
        customController.update(deltaTime);
    };

    console.log('自定义Spine动画初始化完成，已加载骨骼配置数量：', customController.boneConfigMap.size);
				...
				if ((i_16 == 0 && mix == 1) || blend == spine.MixBlend.add) {
					for (var ii = 0; ii < timelineCount; ii++) {
						...
						if (timeline instanceof spine.AttachmentTimeline)
							...
							// AnimationState.prototype.apply 通过 timeline.apply，把动画帧的属性值写入骨骼和插槽对象，实现动画效果。
...
(function (spine) {
	var Bone = (function () {
		...
		Bone.prototype.getWorldScale = function () {
			var n = this.scaleX
				, c = this.scaleY;
			if (this.parent) {
				var d = this.parent.getWorldScale();
				n *= d.x,
				c *= d.y
			}
			return {
...
(function (spine) {
	var SkeletonJson = (function () {
		...
		SkeletonJson.prototype.readSkeletonData = function (json) {
			...
			if (root.extraSlot) {
				skeletonData.extraSlot = root.extraSlot;
			}
			if (root.extraConfig) {
	...
	(function (threejs) {
		...
		var SkeletonMesh = (function (_super) {
			...
			function SkeletonMesh(skeletonData, materialCustomizer) {
				...
				/////////////////////////////////// custom spine //////////////////////////////////
				_this.autoBone = Object.keys(_this.skeleton.data.extra).map(function(P) {
					return _this.skeleton.data.extra[P]
				}).map(function(P) {
					return new CustomAutoBoneAnimator(P, _this)
				});

				// 设置初始动画
				if (_this.skeleton.data.animations && _this.skeleton.data.animations.length > 0) {
					var firstAnimName = _this.skeleton.data.animations[0].name;
					_this.state.setAnimation(0, firstAnimName, true);
				}

				// _this.autoSlot = Object.keys(_this.skeleton.data.extraSlot).map(function(P) {
				// 	return _this.skeleton.data.extraSlot[P]
				// }).map(function(P) {
				// 	return new CustomAutoSlotAnimator(P, _this)
				// });

				// _this.autoBoneSpeed = new AutoBoneSpeedParams(_this.skeleton.data.extraConfig);

				if (_this.skeleton.data.extra) {
					// 初始化自定义动画控制器
					this.customSpineController = new CustomSpineAnimationController(this, _this.skeleton.data.extra);
				}
			...
			SkeletonMesh.prototype.debugBoneInfo = function (boneName) {
				const bone = this.skeleton.findBone(boneName);
				if (!bone) {
					console.warn("Bone not found: " + boneName);
					return;
				}
				console.log(`Bone "${boneName}" 信息:`);
				console.log({
					name: bone.data.name,
					x: bone.x,
					y: bone.y,
					...
					worldScaleX: bone.getWorldScaleX(),
					worldScaleY: bone.getWorldScaleY(),
					parent: bone.parent ? bone.parent.data.name : null
				});
			};

			// 隐藏指定骨骼的所有插槽
			SkeletonMesh.prototype.hideBoneSlots = function (boneName) {
				// 隐藏指定骨骼的所有插槽
			...
			SkeletonMesh.prototype.saveBoneInfo = function (boneName) {
				const bone = this.skeleton.findBone(boneName);
				if (!bone) {
					console.warn("Bone not found: " + boneName);
					return;
				...
				a.href = url;
				a.download = `bone-${boneName}-debug.json`;
			...
			// 每帧驱动动画和骨骼变换的地方
			SkeletonMesh.prototype.update = function (deltaTime, customTime=null) {
				var state = this.state;
				var skeleton = this.skeleton;

				// 用 customTime 推进主动画
				if (customTime !== null) {
					skeleton.time = customTime;
					// 让所有 TrackEntry 的 trackTime 也同步
					if (state.tracks) {
						for (let i = 0; i < state.tracks.length; i++) {
							const entry = state.tracks[i];
							if (entry) entry.trackTime = customTime;
						}
					}
				}

				// state.update(deltaTime); // 推进动画时间
				state.apply(skeleton); // 应用动画到骨骼

				    // 使用自定义动画控制器驱动骨骼
				// if (this.customSpineController) {
				// 	this.customSpineController.update(deltaTime);
				// }

				// this.saveBoneInfo('1'); // 保存名为 '1' 的骨骼信息到文件

				// 驱动自定义骨骼动画
				// if (this.autoBone && this.autoBone.length > 0) {
				// 	const time =  (performance.now() - startTime) / 1000; // 或其它动画时间
				// 	// const time = 1; // 设置一个固定的时间点
				// 	for (const animator of this.autoBone) {
				// 		animator.render(deltaTime, time, 1, null);
				// 	}
				// }

				// 用 customTime 驱动自定义动画
				// if (this.customSpineController) {
				// 	this.customSpineController.update(deltaTime, customTime);
				// }
				if (this.autoBone && this.autoBone.length > 0) {
					const time = customTime !== null ? customTime : skeleton.time;
					for (const animator of this.autoBone) {
						animator.render(deltaTime, time, 1, null);
					}
				}

				// this.hideBoneSlots('toufa59'); // 隐藏名为 '1' 的骨骼的所有插槽

				// this.debugBoneInfo('1'); // 打印名为 '1' 的骨骼信息

				this.skeleton.updateWorldTransform(); // 计算骨骼最终世界坐标
				this.updateGeometry(); // 更新几何体数据
